...
def extract_sequences(tsv_file, genome_file, loci_list_file, output_dir, sample_id, flank_bp, max_hits):
    ...
    # 3. Create indexed genome reader (memory-efficient, lazy loading)
    # Instead of loading entire genome into memory, use indexed access
    try:
    ...
    for locus, group in df.groupby('clean_locus'):
        ...
        for _, row in sorted_group.iterrows():
            # [Key] Stop extracting if hit count limit is reached
            if hits_processed >= max_hits:
                break
                
            sseqid = str(row['sseqid'])
            if sseqid not in genome_index: continue
        ...
        # Write file
        if records:
            out_path = os.path.join(output_dir, f"{locus}.fasta")
