...
rule prepare_sample_queries:
    ...
    [중요 수정] 헤더에 Locus ID(G####)를 추가하여 BLAST 후에도 유전자 정보를 유지함.
    """
    input:
    ...
    run:
        ...
        with open(output.temp_fasta, "w") as out_f:
            ...
            for locus_file in os.listdir(input.probe_dir):
                ...
                # [수정 1] 파일 이름에서 유전자 ID 추출 (예: G4471.AA.fasta -> G4471)
                ...
                for record in SeqIO.parse(locus_path, "fasta"):
                    if record.id.lower().startswith(f"{order_norm}_"):
                        # [수정 2] 헤더 변경: >G4471|Original_Header
                        # 이렇게 해야 BLAST 결과의 qseqid에 G4471이 남습니다.
                        record.id = f"{locus_id}|{record.id}"
                        record.description = record.id # description도 동일하게 맞춤
...
rule blast_queries:
    """
    Step 3.2: Perform tBLASTn (Header contains G####| prefix now)
    """
    input:
...
rule filter_and_tag_hits:
    """
    Step 3.3: Parse BLAST results and create standardized FASTA headers.
    """
    input:
