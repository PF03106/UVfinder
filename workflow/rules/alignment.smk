...
rule mafft_alignment:
    ...
    output:
        aln_ref = temp("results/06_alignment/{type}/aligned/{gene}_with_ref.aln")
    threads: 4
...
rule filter_alignment:
    input:
        ...
        aln_ref = "results/06_alignment/{type}/aligned/{gene}_with_ref.aln"
    output:
        # Final alignment file (Ref removed)
        aln_final = temp("results/06_alignment/{type}/temp_aligned/{gene}.aln")
    params:
        # Pass my sample list
        samples = lambda w: " ".join(SAMPLES)
...
rule trimal_trimming:
    input:
        ...
        aln = "results/06_alignment/{type}/temp_aligned/{gene}.aln"
    output:
